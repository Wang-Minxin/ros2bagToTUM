...
class SidewalkSegmentation(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('Init node.')
        # wmx添加
        self.rgbtxt = open("rgb.txt","w+")
        self.depthtxt = open("depth.txt","w+")
        self.writeHead()
    
    def writeHead(self):
        self.depthtxt.write("# depth maps\n")
        self.depthtxt.write("# file: 'rgbd_dataset_freiburg1_desk2.bag'\n")
        self.depthtxt.write("# timestamp filename\n")
        self.rgbtxt.write("# color images\n")
        self.rgbtxt.write("# file: 'rgbd_dataset_freiburg1_desk.bag'\n")
        self.rgbtxt.write("# timestamp filename\n")
    ...
    def get_filename(self, msg, prefix, extension):
        s = "{:.6f}".format(msg.header.stamp.nanosec / 1000000000)
        s = s[1:8]
        return '{}/{}/{}{}.{}'.format(self.path, prefix, msg.header.stamp.sec, s, extension)
    def saveText(self, msg, prefix):
        s = "{:.6f}".format(msg.header.stamp.nanosec / 1000000000)
        s = s[1:8]
        s = str(msg.header.stamp.sec)+s
        txt = s+" " + prefix+"/"+s+".png\n"
        if(prefix == 'rgb'):
            self.rgbtxt.write(txt)
        elif(prefix == 'depth'):
            self.depthtxt.write(txt)
    ...
    def depth_listener_callback(self, msg):
        ...
        self.saveText(msg, 'depth')

    def color_listener_callback(self, msg):
        self.save_image(msg, 'rgb', 'bgr8')
        self.saveText(msg,'rgb')
